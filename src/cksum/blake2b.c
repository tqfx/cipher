...
static inline void blake2b_set_lastnode(blake2b_s *ctx) { ctx->__f[1] = 0xFFFFFFFFFFFFFFFF; }

static inline int blake2b_is_lastblock(const blake2b_s *ctx) { return (ctx->__f[0] != 0); }
...
static void blake2b_compress(blake2b_s *ctx, const unsigned char *buf)
{
    uint64_t v[0x10];
...
int blake2b_init(blake2b_s *ctx, size_t siz, const void *pdata, size_t nbyte)
{
    ...
    if (pdata)
    {
        ...
        blake2b_proc(ctx, ctx->__buf, sizeof(ctx->__buf));
    }
...
int blake2b_proc(blake2b_s *ctx, const void *pdata, size_t nbyte)
{
    ...
    if (nbyte > n)
    {
        ...
        blake2b_increment_counter(ctx, sizeof(ctx->__buf));
        blake2b_compress(ctx, ctx->__buf);
        ctx->__cursiz = 0;
        nbyte -= n;
        p += n;
...
unsigned char *blake2b_done(blake2b_s *ctx, void *out)
{
    ...
    blake2b_increment_counter(ctx, ctx->__cursiz);
